...
int nTotalSockets = 0;
SOCKETINFO* SocketInfoArray[FD_SETSIZE];//포인터 배열
...
int main(int argc, char* argv[])
{
	...
	// SO_REUSEADDR 옵션 설정
	BOOL optval = TRUE;
	retval = setsockopt(listen_sockv4, SOL_SOCKET,
...
void RemoveSocketInfo(int nIndex)
{
	...
	if (nIndex != (nTotalSockets - 1))
		SocketInfoArray[nIndex] = SocketInfoArray[nTotalSockets - 1];//제일 마지막꺼를 빈 곳에 넣는다

	--nTotalSockets;//전체 크기는 1감소시킴
}
